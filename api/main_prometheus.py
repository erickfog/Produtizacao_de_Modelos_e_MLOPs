...
REQUEST_COUNT = Counter(
    'api_requests_total', 'Total de requisições para a API', ['method', 'endpoint']
)

REQUEST_LATENCY = Histogram(
    'api_request_duration_seconds', 'Duração das requisições para a API', ['endpoint']
)
...
def predict():
    ...
    try:
        ...
        REQUEST_LATENCY.labels(endpoint="/predict").observe(time() - start_time)

@app.route("/metrics")
def metrics():
    return generate_latest(REGISTRY)

# Integrar o WSGI do Prometheus com o Flask
